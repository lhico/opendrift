...
from opendrift.readers.roms_grid import work_grid
...
class Reader(BaseReader, StructuredReader):
	...
	def __init__(self, filename=None, name=None, gridfile=None, standard_name_mapping={}):
		...
		self.zlevels = np.array([-5, -10,-15,-20,-25,-30,-35,-40,-50,-60,-65,-70,-75,-80,-85,-90,-95,-100,-120,-150,
			-175,	-250, -300, -400, -500, -600, -700, -800, -900, -1000,-1200, -1500,-1800,
		...
		if isinstance(filename, xr.Dataset):
			...
			try:
				...
				self.Dataset = work_grid.fix_angle(self.Dataset.copy())
		...
		if dimensions == 3:
			...
			self.Cs_r = np.array([-9.75535223e-01, -9.21857481e-01, -8.63070756e-01, -8.00808166e-01,
				-7.36738441e-01, -6.72426304e-01, -6.09236951e-01, -5.48283952e-01,
				-4.90413483e-01, -4.36215138e-01, -3.86049597e-01, -3.40084938e-01,
				-2.98335442e-01, -2.60698778e-01, -2.26989165e-01, -1.96965403e-01,
				-1.70353535e-01, -1.46864460e-01, -1.26207076e-01, -1.08097690e-01,
				-9.22663919e-02, -7.84610649e-02, -6.64495976e-02, -5.60207813e-02,
				-4.69842707e-02, -3.91699112e-02, -3.24266608e-02, -2.66212796e-02,
				-2.16369095e-02, -1.73716343e-02, -1.37370806e-02, -1.06570992e-02,
				-8.06655299e-03, -5.91022448e-03, -4.14185161e-03, -2.72329215e-03,
				-1.62381664e-03, -8.19526705e-04, -2.92895366e-04, -3.24258270e-05])
		else:
			self.num_layers = 1
...
###########----Working with z:-------##################################
		...
		if z.min() == 0 or not hasattr(self, 'hc'):
			...
			print('indz ==',indz)
			print("VAR_Z ==", variables['z'])
			print('len of var_z', len(variables['z']))
...
##################################################################################################################################
		...
		if 'x_sea_water_velocity' in variables.keys() or \
			...
			if 'angle' in self.Dataset.variables:
				logger.debug('Reading angle between xi and east from ROMS grid')
				self.angle_xi_east = self.Dataset.variables['angle'][:]
				rad = self.angle_xi_east[indy, indx]
				rad = np.ma.asarray(rad)
			else:
				self.angle_xi_east = np.deg2rad(-30)
				logger.debug('setting angle from mean direction as -30º')
